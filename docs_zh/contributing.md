...
如果您不知道从哪里开始，有一个特殊的[Good First
问题](https://github.com/huggingface/transformers/contribute)清单。它会为您提供一个列表
开放问题，适合初学者，并帮助您开始贡献开源。只需在问题中发表评论，表示您想要在该问题上工作。

如果您愿意面临一些更有挑战性的事情，您也可以查看[Good Second Issue](https://github.com/huggingface/transformers/labels/Good%20Second%20Issue)列表。通常情况下，如果您认为您知道自己在做什么，那就去做吧，我们会帮助您取得成就！🚀

> 所有贡献对社区同等有价值。🥰

## 解决未解决的问题

如果您注意到现有代码的问题，并想法来解决，可以自由地[开始贡献](https://github.com/huggingface/transformers/blob/main/CONTRIBUTING.md/#create-a-pull-request)并打开拉取请求！

## 提交与错误相关的问题或功能请求

在提交与错误相关的问题或功能请求时，请尽力遵循以下准则。这将使我们更容易迅速地给您提供反馈。

### 您发现了错误吗？

感谢使用者报告他们遇到的问题，🤗 Transformers 库非常健壮可靠。

在报告问题之前，我们非常希望您 **确保未曾报告过该错误**
（在 GitHub 的 Issues 下使用搜索栏）。您的问题应与库本身的问题相关，并且不应与您的代码相关。如果您不确定错误是否在您的代码或库中，请先在[论坛](https://discuss.huggingface.co/)上询问。这样，我们将能够更快地回应与库相关的问题而不是一般问题。

确认错误尚未报告过后，请在您的问题中包含以下信息，以便我们能够快速解决它：

* 当适用时，请提供您的 **操作系统类型和版本**、**Python 版本**、**PyTorch 版本** 和 **TensorFlow 版本**。
...
如果您希望在 🤗 Transformers 中看到新功能，请打开一个问题并描述：

1. 此功能背后的**动机**是什么？它是否与库的问题或困扰有关？它是否与您为某个项目所需的功能有关？是否与您工作过的功能有关，您认为它可能对社区有所帮助？

   无论是什么，我们都很乐意听听您的想法！

2. 尽可能详细地描述您请求的功能。您提供的信息越详细，我们就能够提供更好的帮助。
3. 提供演示功能用法的 *代码片段*。
4. 如果该功能与一篇论文有关，请包含链接。

如果您写得足够好，我们在创建它时就已经完成了 80%。

我们添加了[模板](https://github.com/huggingface/transformers/tree/main/templates)来帮助您开始处理问题。

## 您想要实现一个新模型吗？

不断有新模型发布，如果您想要实现一个新模型，请提供以下信息

* 模型的简要描述和论文链接。
* 如果模型是开源的，请提供实现链接。
* 如果模型权重可用，请提供权重链接。

如果您愿意贡献模型，请告诉我们，好让我们能协助您将其添加到 🤗 Transformers！

我们已经添加了一个[详细指南和模板](https://github.com/huggingface/transformers/tree/main/templates)，以帮助您开始添加新模型，我们还有一份更详细的指南，[如何将模型添加到🤗 Transformers](https://huggingface.co/docs/transformers/add_new_model)。

## 您想要添加文档吗？

我们始终在寻求对文档进行改进，使其更加清晰和准确。请告诉我们文档如何改进，例如拼写错误和缺少的内容，以及不清楚或不准确的内容。如果您有兴趣，我们很乐意进行更改或协助您进行贡献！

有关如何生成、构建和编写文档的详细信息，请参阅文档[README](https://github.com/huggingface/transformers/tree/main/docs)。

## 创建拉取请求

在编写任何代码之前，我们强烈建议您搜索现有的 PR 或问题，
以确保没有其他人正在处理同样的事情。如果您不确定，最好是打开一个问题以获取一些反馈。

要贡献到
🤗 Transformers，你需要基本的 `git` 熟练技能。
虽然 `git` 不是最容易使用的工具，但它有最好的
帮助手册。在 shell 中输入 `git --help` 并享受吧！如果您更喜欢阅读书籍，那么 [Pro
Git](https://git-scm.com/book/en/v2) 是一个非常好的参考。

您需要 **[Python 3.8]((https://github.com/huggingface/transformers/blob/main/setup.py#L426))** 或更高的版本来贡献到 🤗 Transformers。请按照以下步骤开始贡献：

1. 通过单击存储库页面上的**[Fork](https://github.com/huggingface/transformers/fork)**按钮，对[存储库](https://github.com/huggingface/transformers)进行分支。这将在您的 GitHub 用户帐户下创建代码的副本。
...
4. 在虚拟环境中运行以下命令，设置开发环境：
   ...
   根据您的操作系统，并且由于 Transformers 的可选依赖项数量在不断增加，您可能会遇到此命令的失败。如果是这种情况，请确保您安装了您正在使用的深度学习框架（PyTorch、TensorFlow 和/或 Flax），然后执行以下操作：
...
5. 在您的分支上开发功能。
   ...
   该目标还经过优化，只与您正在处理的 PR 所修改的文件一起工作。

   如果您愿意依次运行各个检查，请使用以下命令应用
   样式更正：
   ...
   质量控制是由 CI 运行的，但是您可以使用相同的检查运行：

   ```bash
   make quality
   ```

   最后，我们有很多脚本，用于确保在添加新模型时不会忘记更新某些文件。您可以使用以下命令运行这些脚本：
   ...
   如果您正在修改 `docs/source` 目录下的文档，请确保文档仍然可以构建。当您打开拉取请求时，CI 也将运行此检查。要运行本地检查，请确保安装了文档生成器：
   ...
   这将在 `~/tmp/test-build` 文件夹中构建文档，您可以使用喜欢的编辑器查看生成的 Markdown 文件。您还可以在打开拉取请求时在 GitHub 上预览文档。

   一旦您对更改感到满意，请使用 `git add` 添加已更改的文件，并在本地记录更改：
   ...
   请记住编写[良好的提交
   消息](https://chris.beams.io/posts/git-commit/)以清楚表达您所做的更改！

   在将您的代码副本与原始存储库保持同步之前，请基于 `upstream/branch` 重派基于您的分支属于该分支的情况下的拉取请求。或者，在维护者的要求下切换分支：
   ...
   如果您已经打开了拉取请求，则需要使用 `--force` 标志强制推送。否则，如果尚未打开拉取请求，则可以像平常一样推送您的更改。

6. 现在，您可以前往您在 GitHub 上的存储库分支，并单击**拉取请求**以打开拉取请求。请确保您在下面的[清单](https://github.com/huggingface/transformers/blob/main/CONTRIBUTING.md/#pull-request-checklist)中选中所有框。准备好后，您可以将更改发送给项目维护者进行审查。

7. 如果维护者请求更改，没关系，我们的核心贡献者也会这样！为了让所有人能够看到拉取请求中的更改，在本地分支上工作并将更改推送到您的存储库。它们将自动出现在
利用 [Changes](https://github.com/huggingface/transformers/blob/main/templates/changes.md) 和相关文件。

### 拉取请求清单

☐ 拉取请求标题应概括您的贡献。<br>
☐ 如果您的拉取请求与问题有关，请在拉取请求说明中提及问题编号，以确保它们链接在一起（查看问题的人将知道您正在处理此问题）。<br>
☐ 要指示进展中的工作，请在标题前缀中添加 `[WIP]`。这些很有用，以避免重复工作，并将其与准备合并的 PR 区分开。<br>
☐ 确保现有测试通过。<br>
☐ 如果添加新功能，请为其添加测试。<br>
   - 如果您要添加新模型，请确保使用
     `ModelTester.all_model_classes = (MyModel, MyModelWithLMHead,...)` 来触发常见测试。
   - 如果您添加了新的 `@slow` 测试，请确保使用
     `RUN_SLOW=1 python -m pytest tests/models/my_new_model/test_my_new_model.py` 运行以进行运行。
   - 如果您要添加新的分词器，请编写测试，并确保
...
您可以在该数据集仓库上打开 PR，并请 Hugging Face 成员合并它。
