...
得到一个隐藏状态表示的序列。然后，MusicGen通过这些隐藏状态来预测离散的音频token或音频码，并使用音频压缩模型（如EnCodec）解码得到音频波形。
...
*我们解决了条件音乐生成任务。我们介绍了MusicGen，一个单阶段的语言模型(LM)，用于多个音乐离散压缩表示流，即token。与之前的工作不同，
...
2. MusicGen解码器：一种语言模型（LM），根据编码器隐藏状态表示自回归地生成音频token（或码）。
3. 音频编码器/解码器：用于对音频提示进行编码以生成提示标记，并从解码器预测的音频token中恢复音频波形。
